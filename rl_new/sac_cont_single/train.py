...
from rl_new.sac_cont_single.sac_utils import setup_devices, create_update_fn
...
def main(cfg: DictConfig):
    with tempfile.TemporaryDirectory(cfg.buffer.temp_dir) as tmpdir: # 使用临时目录存储内存映射文件
        ...
        # 设备配置
        train_device, collector_device = setup_devices(cfg)
        torchrl_logger.info(f"训练设备: {train_device}")
        torchrl_logger.info(f"收集设备: {collector_device}")
        ...
        # ============ 创建收集器（使用aSyncDataCollector单进程） ============
        # 环境运行在CPU，策略推理可以在GPU
        env_device = 'cpu'
        
        collector = aSyncDataCollector(
            create_env_fn=lambda: make_env(
                num_envs=cfg.collector.num_envs,  # 单进程内并行环境数
                device=env_device
            ),
            policy=actor,
            frames_per_batch=cfg.collector.frames_per_batch,
            total_frames=cfg.collector.total_frames,
            device=collector_device,      # 策略推理设备(GPU或CPU)
            env_device=env_device,         # 环境运行设备(CPU)
        ...
        collector.start()
        torchrl_logger.info(f"创建单进程异步收集器 (aSyncDataCollector) - 环境数: {cfg.collector.num_envs}")

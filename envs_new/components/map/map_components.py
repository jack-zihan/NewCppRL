...
class ObstacleCreator:
    ...
    def _is_valid_placement(self, obstacle: np.ndarray, agent_position: Tuple[float, float],
        ...
        # 使用int()进行网格索引（floor操作），获取中心所在的网格
        ...
        # cv2.pointPolygonTest测点在多边形内部的距离，为负则表示点在多边形外部，大于 2*0 *agent_length的位置
...
class WeedCreator:
    ...
    def generate(self, state: Dict[str, Any], rng: np.random.Generator) -> None:
        ...
        # 判断是否从目录加载
        if options.get('scenario_directory'):
            weed_map = self._load_from_directory(options['scenario_directory'], state['dimensions'])
        else:
            weed_map = self._generate_weed_distribution( state['maps_dict']['field_frontier'],
                options.get('weed_distribution', 'uniform'), options.get('weed_count', 100), rng)

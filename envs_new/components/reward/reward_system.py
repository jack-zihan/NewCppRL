...
class TurningPenaltyCalculator(RewardCalculator):
    ...
    def calculate(cls, env_state: EnvironmentState, coefficient: float, config: EnvironmentConfig = None, **kwargs) -> float:
        ...
        if not steer_info or steer_info.last is None: return 0.0
...
class OverlapPenaltyCalculator(RewardCalculator):
    """重复覆盖惩罚：对每步新增的重叠格子数施加惩罚。

    归一化使用机器人本体投影面积 scale = agent_width * agent_length，
    与 FieldCoverage 的推进带宽形成对称。
    """

    @classmethod
    def calculate(cls, env_state: EnvironmentState, coefficient: float, config: EnvironmentConfig = None, **kwargs) -> float:
        overlap_count = env_state.get_info('overlap_count')
        if not overlap_count: return 0.0
        delta_overlap = float(overlap_count.change())
        assert delta_overlap >= 0
        return - (delta_overlap / (2 * max(config.agent_width * config.v_max, 1e-6))) * abs(coefficient) # 覆盖方式改为前沿覆盖了，因此归一化因子变为理论最大覆盖区域
        # 这里照道理量纲应该同field coverage一样是面积，但实际上是个count，暂时先这样
...
class RewardSystem:
    ...
    AVAILABLE_CALCULATORS = {
        ...
        'overlap_penalty': OverlapPenaltyCalculator,
    }

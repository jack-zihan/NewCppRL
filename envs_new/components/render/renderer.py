...
class Renderer:
    ...
    def _render_map(self, maps_dict: Dict[str, np.ndarray], agent: Agent,
        ...
        cv2.ellipse(img=rendered_map, center=agent.position_discrete, angle=agent.direction,
            ...
            startAngle=-int(agent.vision_angle / 2), endAngle=int(agent.vision_angle / 2),
            color=RENDER_COLORS['agent_vision'], thickness=-1
        )
        ...
        # 应用mist效果（对未探索区域mist=0应用雾效）
        if self.config.render_mist and 'mist' in maps_dict:
            unexplored = np.logical_not(maps_dict['mist']).astype(bool)
    ...
    def _render_weeds(self, rendered_map: np.ndarray,
        ...
        # 渲染已清除的杂草
        if (self.config.render_covered_weed and 'original_weed' in maps_dict):
            weed_covered = enlarge_map_features(np.logical_and(maps_dict['original_weed'],np.logical_not(weed_map)))
            if weed_covered.any():
                rendered_map[weed_covered] = ((1 - COVERED_WEED_ALPHA) * np.array(RENDER_COLORS['weed_covered']) +
                        COVERED_WEED_ALPHA * rendered_map[weed_covered]).astype(np.uint8)
    ...
    def _render_first_person(self, maps_dict: Dict[str, np.ndarray], agent: Agent,
        ...
        # 提取第一人称补丁 - 使用共享工具函数
        patch = extract_ego_patch(maps=base_map, pad_values=[128] * 3,  # RGB channels with gray padding
            center_y=agent.y, center_x=agent.x, direction_deg=agent.direction, patch_size=observation_size)

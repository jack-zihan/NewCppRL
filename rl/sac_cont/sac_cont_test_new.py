...
class SACTestSession:
    ...
    def _setup_environment(self):
        ...
        self._verify_wrapper_compatibility()
    
    def _verify_wrapper_compatibility(self):
        """Verify that wrappers are properly configured and compatible."""
        # Check if both wrappers are active
        if self.config['log_reward'] and self.config['log_feature']:
            print("Warning: Both RewardTracker and FeatureTracker are active.")
            print("Wrapper order: RewardTracker → FeatureTracker → Base Environment")
            print("This may cause slight performance overhead due to duplicate calculations.")
        
        # Verify RewardTracker is properly applied
        if self.config['log_reward']:
            if not hasattr(self.env, 'get_episode_summary'):
                raise RuntimeError("RewardTracker wrapper not properly applied")
            print("✓ RewardTracker wrapper verified")
        
        # Verify FeatureTracker is properly applied  
        if self.config['log_feature']:
            # Check for FeatureTracker methods
            if not hasattr(self.env, 'get_current_features'):
                raise RuntimeError("FeatureTracker wrapper not properly applied")
            print("✓ FeatureTracker wrapper verified")
        
        # Additional compatibility checks when both are active
        if self.config['log_reward'] and self.config['log_feature']:
            # Warn about potential state modification timing
            print("\nNote: FeatureTracker modifies environment state (use_apf) during feature capture.")
            print("This happens AFTER RewardTracker has captured reward data.")
            print("If reward calculations depend on use_apf state, there may be inconsistencies.")

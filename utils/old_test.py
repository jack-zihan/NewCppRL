...
while True:
    ...
    if len(trajectory) >= 2:
        # 将轨迹点转换为形状 (N, 1, 2) 的数组
        trajectory_points = np.array(trajectory, dtype=np.float32).reshape(-1, 1, 2)

        # 将轨迹点从初始帧坐标系转换回当前帧坐标系
        inv_cumulative_transform = np.linalg.inv(cumulative_transform)
        trajectory_points_current = cv2.perspectiveTransform(trajectory_points, inv_cumulative_transform)

        # 绘制轨迹
        for i in range(1, len(trajectory_points_current)):
            pt1 = (int(trajectory_points_current[i - 1][0][0]), int(trajectory_points_current[i - 1][0][1]))
            pt2 = (int(trajectory_points_current[i][0][0]), int(trajectory_points_current[i][0][1]))
